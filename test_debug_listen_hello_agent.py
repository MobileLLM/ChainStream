...
def test_debug_listen_hello_agent():
    ...
    try:
        ...
        # 启动DebugHelloAgent
        logger.info("🏃 启动DebugHelloAgent")
        debug_hello_agent_path = os.path.abspath("AgentStore/system_agents/utils/DebugHelloAgent.java")
        
        if not os.path.exists(debug_hello_agent_path):
            logger.error(f"❌ DebugHelloAgent文件不存在: {debug_hello_agent_path}")
            return False
        
        success = runtime_core.start_agent_by_path(debug_hello_agent_path, user)
        if not success:
            logger.error("❌ 启动DebugHelloAgent失败")
            return False
        
        logger.info("✅ DebugHelloAgent启动成功")
        ...
        if len(java_agents) < 2:
            logger.error(f"❌ 期望2个Java Agent，实际找到{len(java_agents)}个")
            return False
