...
class WeatherTask6(SingleAgentTaskConfigBase):
    def __init__(self):
        ...
        self.output_stream_description = StreamListDescription(streams=[
            {
                "stream_id": "Precipitation_over_5_mm",
                "description": "A stream of the precipitation of the zones if the precipitation is over 5mm",
                "fields": {
                    "Precipitation_over_5_mm": "The precipitation of the zone which is over 5mm, float",
                    "Location": "The location of the zone, string",
                    "Date_Time": "The time of the zone with the format of '%Y/%m/%d %H:%M', datetime"
                }
            }
...
class testAgent(cs.agent.Agent):
        ...
        def process_weather(weather):
            ...
            if Precipitation_mm > 5:         
                self.output_stream.add_item({
                    "Precipitation_over_5_mm":Precipitation_mm,
                    "Location": location,
                    "Date_Time": time
                })
    ...
    def init_environment(self, runtime):
        ...
        self.output_weather_stream = cs.stream.create_stream(self, 'Precipitation_over_5_mm')
        self.output_record = {x.stream_id: [] for x in self.output_stream_description.streams}

        def record_output(data):
            self.output_record['Precipitation_over_5_mm'].append(data)
    ...
    def init_output_stream(self, runtime):
        self.output_weather_stream = cs.stream.get_stream(self, 'Precipitation_over_5_mm')
        self.output_record = {x.stream_id: [] for x in self.output_stream_description.streams}
