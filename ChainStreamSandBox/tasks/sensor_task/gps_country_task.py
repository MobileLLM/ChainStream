...
class GPSTask9(SingleAgentTaskConfigBase):
    def __init__(self):
        ...
        self.output_stream_description = StreamListDescription(streams=[
            {
                "stream_id": "gps_country_in_French",
                "description": "A stream of the country name translated into French extracted from the gps data",
                "fields": {
                    "country_French": "The name of the country translated into French, string"}
            }
        ])
...
class testAgent(cs.agent.Agent):
    def __init__(self):
        ...
        self.output_stream = cs.create_stream(self,"gps_country_in_French")
    ...
    def init_environment(self, runtime):
        ...
        self.output_gps_stream = cs.stream.create_stream(self, 'gps_country_in_French')
        self.output_record = {x.stream_id: [] for x in self.output_stream_description.streams}

        def record_output(data):
            self.output_record['gps_country_in_French'].append(data)
    ...
    def init_output_stream(self, runtime):
        self.output_gps_stream = cs.stream.get_stream(self, 'gps_country_in_French')
        self.output_record = {x.stream_id: [] for x in self.output_stream_description.streams}
