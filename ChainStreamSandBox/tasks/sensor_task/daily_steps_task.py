...
class HealthTask10(SingleAgentTaskConfigBase):
    def __init__(self):
        ...
        self.output_stream_description = StreamListDescription(streams=[
            {
                "stream_id": "daily_steps_less_than_1k",
                "description": "A stream of the steps daily which is less than 1000",
                "fields": {
                    "daily_steps_less_than_1k": "The steps calculated daily which is less than 1000, int"}
            }
        ])
...
class testAgent(cs.agent.Agent):
    def __init__(self):
        ...
        self.output_stream = cs.create_stream(self,"daily_steps_less_than_1k")
        ...
        def process_health(health):
            ...
            if Daily_Steps < 1000:  
                self.output_stream.add_item({
                    "daily_steps_less_than_1k": Daily_Steps
                    })
    ...
    def init_environment(self, runtime):
        ...
        self.output_health_stream = cs.stream.create_stream(self, 'daily_steps_less_than_1k')
        self.output_record = {x.stream_id: [] for x in self.output_stream_description.streams}

        def record_output(data):
            self.output_record['daily_steps_less_than_1k'].append(data)
    ...
    def init_output_stream(self, runtime):
        self.output_health_stream = cs.stream.get_stream(self, 'daily_steps_less_than_1k')
        self.output_record = {x.stream_id: [] for x in self.output_stream_description.streams}
