...
class HealthTask6(SingleAgentTaskConfigBase):
    def __init__(self):
        ...
        self.input_stream_description = StreamListDescription(streams=[{
            ...
            "fields": {
                "Physical Activity Level": "The level of the physical exercise, int"
            }
        }])
        self.output_stream_description = StreamListDescription(streams=[
            {
                "stream_id": "exercise_level_sorted",
                "description": "A stream of the physical exercise level sorted in descending order with every six "
                               "copies of streams grouped as a batch",
                "fields": {
                    "exercise_level": "The level of the physical exercise, int"}
            }
        ])
...
class testAgent(cs.agent.Agent):
    def __init__(self):
        ...
        self.output_stream = cs.create_stream(self,"exercise_level_sorted")
        ...
        def process_health(health_data):
            ...
            for health in sorted_list: 
                self.output_stream.add_item({
                    "exercise_level": health['Physical Activity Level']
                    })
    ...
    def init_environment(self, runtime):
        ...
        self.output_health_stream = cs.stream.create_stream(self, 'exercise_level_sorted')
        self.output_record = {x.stream_id: [] for x in self.output_stream_description.streams}

        def record_output(data):
            self.output_record['exercise_level_sorted'].append(data)
    ...
    def init_output_stream(self, runtime):
        self.output_health_stream = cs.stream.get_stream(self, 'exercise_level_sorted')
        self.output_record = {x.stream_id: [] for x in self.output_stream_description.streams}
