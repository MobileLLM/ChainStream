...
class TweetTask7(SingleAgentTaskConfigBase):
    def __init__(self):
        ...
        self.output_stream_description = StreamListDescription(streams=[
            {
                "stream_id": "tweets_text_in_French",
                "description": "The texts of the tweet which are translated into French",
                "fields": {
                    "French_text": "The text of the tweet translated into French, string",
                }
            }
        ])
...
class testAgent(cs.agent.Agent):
    def __init__(self):
        ...
        self.output_stream = cs.create_stream(self,"tweets_text_in_French")
        self.llm = get_model("Text")
    def start(self):
        def process_tweet(tweets):
            tweets_text = tweets["text"]
            prompt = 'Please translate the following text into French'
            res = self.llm.query(cs.llm.make_prompt(prompt, tweets_text))
    ...
    def init_environment(self, runtime):
        ...
        def record_output(data):
            self.output_record['tweets_text_in_French'].append(data)
    ...
    def init_output_stream(self, runtime):
        self.output_tweet_stream = cs.stream.get_stream(self, 'tweets_text_in_French')
        self.output_record = {x.stream_id: [] for x in self.output_stream_description.streams}
