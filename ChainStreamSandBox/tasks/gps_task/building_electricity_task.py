...
class BuildingElectricityConfig(SingleAgentTaskConfigBase):
    def __init__(self):
        super().__init__()
        ...
        self.task_description = (
            "Retrieve data from the input stream 'all_landmarks' and process the values corresponding to the 'Electricity(kWh)' key in the landmark dictionary: "
            "Add the electricity(kWh) number to the output stream 'cs_landmarks'."
        )
        ...
        class testAgent(cs.agent.Agent):
            ...
            def start(self):
                def process_landmark(landmark):
                    Electricity(kWh) = landmark["Electricity(kWh)"]        
                    self.output_stream.add_item(Electricity(kWh))
...
if __name__ == '__main__':
    config = BuildingElectricityConfig()
