...
class BuildingNaturalGasConfig(SingleAgentTaskConfigBase):
    def __init__(self):
        ...
        self.task_description = (
            "Retrieve data from the input stream 'all_landmarks' and process the values corresponding to the 'NaturalGas(therms)' key in the landmark dictionary: "
            "Add the NaturalGas(therms) number to the output stream 'cs_landmarks'."
        )
        ...
        class testAgent(cs.agent.Agent):
            ...
            def start(self):
                def process_landmark(landmark):
                    NaturalGas(therms) = landmark["NaturalGas(therms)"]        
                    self.output_stream.add_item(NaturalGas(therms))
