...
class BuildingGHGEmissionsConfig(SingleAgentTaskConfigBase):
    def __init__(self):
        ...
        self.task_description = (
            "Retrieve data from the input stream 'all_landmarks' and process the values corresponding to the 'GHGEmissions(MetricTonsCO2e)' key in the landmark dictionary: "
            "Add the GHGEmissions(MetricTonsCO2e) number to the output stream 'cs_landmarks'."
        )
        ...
        class testAgent(cs.agent.Agent):
            ...
            def start(self):
                def process_landmark(landmark):
                    GHGEmissions(MetricTonsCO2e) = landmark["GHGEmissions(MetricTonsCO2e)"]        
                    self.output_stream.add_item(GHGEmissions(MetricTonsCO2e))
