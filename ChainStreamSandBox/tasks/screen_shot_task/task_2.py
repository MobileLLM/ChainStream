...
class ScreenshotTask2(SingleAgentTaskConfigBase):
    def __init__(self):
        super().__init__()
        ...
        self.output_stream_description = StreamListDescription(streams=[
            {
                ...
                "description": "The analysis of the screenshot properties on whether the root view is scrollable "
                               "horizontally",
                "fields": {
...
class testAgent(cs.agent.Agent):
    def __init__(self):
        ...
        self.output_stream = cs.create_stream(self,"screenshot_details")
        self.llm = get_model("image")
    def start(self):
        def process_screenshot(screenshot):
            root_info = screenshot['info']['activity']['root']
            scrollable_horizontal = root_info.get('scrollable-horizontal')
            self.output_stream.add_item({
            "scrollable_horizontal": scrollable_horizontal})
