...
class EmailTask10(SingleAgentTaskConfigBase):
    def __init__(self):
        super().__init__()
        ...
        self.output_stream_description = StreamListDescription(streams=[
            {
                "stream_id": "emails_subject",
                "description": "A stream of emails with the extraction of the subject",
                "fields": {
                    "Subject": "The subject of the email, string"}
            }
        ])
...
class testAgent(cs.agent.Agent):
    def __init__(self):
        ...
        self.output_stream = cs.create_stream(self,"emails_subject")
    ...
    def init_environment(self, runtime):
        ...
        self.output_email_stream = cs.stream.create_stream(self, 'emails_subject')

        self.output_record = {x.stream_id: [] for x in self.output_stream_description.streams}

        def record_output(data):
            self.output_record['emails_subject'].append(data)
    ...
    def init_output_stream(self, runtime):
        self.output_email_stream = cs.stream.get_stream(self, 'emails_subject')
