...
class EvaluatorBase:
    def __init__(self, log_path_list: list, save_name=None, save_folder=None):
        ...
        self.save_path = os.path.join(
            self.save_folder, datetime.datetime.now().strftime("%Y%m%d_%H%M%S") + "_" + self.save_name + ".json") \
            ...
            for task, report_list in log['task_reports'].items():
                ...
                for report in report_list:
                    ...
                    if "error_msg" in report and report["error_msg"] == "success":
                        task_report_data[task].append(_check_and_load_task_report(report["report_path"]))
