...
class EdgeRecoder:
    def __init__(self, stream_id, agent_file_path=None, func_id=None, agent_id=None, type='agent_to_queue', analysis_pre_min=10):
    ...
    def record_new_item(self):
        ...
        if self.statistics[-1][0] > time_now - datetime.timedelta(minutes=self.analysis_pre_min):
            self.statistics.append(
                (time_now, len(self.gap_log) / self.analysis_pre_min))
            self.gap_log = []
        self.gap_log.append(time_now)

    def get_agent_to_queue_log(self):
        if self.type != 'agent_to_queue':
            return {self.agent_id: self.log}
        return {(self.agent_file_path, self.func_id): self.log}

    def get_agent_to_queue_statistics(self):
        if self.type != 'agent_to_queue':
            return {self.agent_id: self.statistics}
        return {(self.agent_file_path, self.func_id): self.statistics}
...
class QueueRecoder:
    ...
    def record_new_item(self):
        self.tot += 1
        time_now = datetime.datetime.now()
        if self.len_statistics[-1][0] > time_now - datetime.timedelta(minutes=self.analysis_pre_min):
            self.len_statistics.append(
                (time_now, sum(self.len_gap_log) / len(self.len_gap_log) / self.analysis_pre_min))
        self.len_gap_log.append(len(self.queue))

    def get_queue_statistics(self):
        return self.len_statistics
...
class StreamRecorder:
    ...
    def record_new_item(self, from_file, from_function):
        if (from_file, from_function) not in self.agent_file_func_to_queue_recorder:
            self.agent_file_func_to_queue_recorder[(from_file, from_function)] = EdgeRecoder(
                self.streamMetaData.stream_id, agent_file_path=from_file, func_id=from_function, type='agent_to_queue', analysis_pre_min=self.analysis_gap)
    ...
    def record_send_item(self, agent_id):
        if agent_id not in self.queue_to_agent_id_func_recorder:
            self.queue_to_agent_id_func_recorder[agent_id] = EdgeRecoder(
                self.streamMetaData.stream_id, agent_id=agent_id, type='queue_to_agent', analysis_pre_min=self.analysis_gap)
    ...
    def get_record_data(self, need_log=False):
        return {
            ...
            "agent_to_queue": {
                (k[0], k[1]): {
                    "statistics": v.get_agent_to_queue_statistics(),
                    "tot": v.get_total_count(),
                    "log": v.get_agent_to_queue_log() if need_log else None
            ...
            "listener_statistics": self.listener_pre_gap_min
        }
